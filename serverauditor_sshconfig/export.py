...
class ExportApplication(object):
    ...
    def _greeting(self):
        self._logger.log("ServerAuditor's ssh config script.", color='magenta')
        return
    ...
    def _get_hosts(self):
        self._logger.log("The following new hosts have been founded in your ssh config:")
        self._logger.log(self._hosts, sleep=0)
        number = None
        while number != '=':
...
def main():
    app = ExportApplication(api=API(), ssh_config=SSHConfig(), cryptor=RNCryptor(), logger=PrettyLogger())
    try:
        app.run()
